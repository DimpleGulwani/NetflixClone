...
window.onload = () => {
 ...
 async function fetchMovies(url, dom_element, path_type) {
    ...
    if(response.ok)
    {
        ...
        showMovies(jsondata,dom_element,path_type)

    }
    else{
        throw new Error('something is wrong with the site');
    }
  ...
  showMovies = (movies, dom_element, path_type) => {
   ...
   let allMovies =document.querySelector(dom_element)
    ...
    imageElement.addEventListener('click', e => {
     ...
     allMovies.appendChild(imageElement)
    }
...
//  fetchMovies('https://api.themoviedb.org/3/discover/tv?api_key=19f84e11932abbc79e6d83f82d6d1045&with_networks=213','.original__movies','poster_path') 
  ...
  async function getMovieTrailer(id) {
    const trailer= `https://api.themoviedb.org/3/movie/${id}/videos?api_key=19f84e11932abbc79e6d83f82d6d1045&language=en-US`
